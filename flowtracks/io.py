...
def trajectories_ptvis(fname, first=None, last=None, frate=1.):
    """
    ...
    table = np.loadtxt(fname % frame_nums[0], dtype=fmt, skiprows=1)
    ...
    for fix, frame_num in enumerate(frame_nums[1:]):
        table = np.loadtxt(fname % frame_num, dtype=fmt, skiprows=1)
        
        # Continue existing trajectories into this frame:
        cont = table['prev'] > -1
        traj = np.empty(table['prev'].shape)
        prev_ix = table['prev'][cont]
        ...
        frame = np.hstack((table['pos']/1000., np.zeros_like(table['pos']), 
            np.ones((table.shape[0], 1))*frame_num, traj[:,None]))
        frames[fix][prev_ix,3:6] = \
            (frame[cont,:3] - frames[fix][prev_ix,:3]) * frate
