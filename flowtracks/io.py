...
def collect_particles_mat(fname, frame, path_seg=False):
    ...
    trajects = data[data_name][:,0]
    
    selected = []
    for traj in trajects:
        # Just a Q&D transformation, not very efficient.
        table = np.hstack((traj['xf'], traj['yf'], traj['zf'],
            traj['uf'], traj['vf'], traj['wf'], traj['t'], traj['trajid']))
        
        if path_seg is True:
            t = np.nonzero((traj['t'] == frame) & \
                (np.roll(traj['t'], -1) == frame + 1))[0]
            if len(t) == 0: continue
            
            sel = np.concatenate(
                (table[t[0],:][None,None,:], table[t[0] + 1,:][None,None,:]),
                axis=0)
            
        else:
            t = np.nonzero(traj['t'] == frame)[0]
            if len(t) == 0: continue
            sel = table[t[0]]
        
        
        sel[...,:6] /= 1000. # convert data from mm to m
        selected.append(sel)
